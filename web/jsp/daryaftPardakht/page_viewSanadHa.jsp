...
<display:table export="true" id="sanadListTbl" uid="row" name="sanadListPaginated"
    ...
    <display:column title="نوع سند" style="">${row.noeSanad == "GHABZE_RESID"? "قبض رسید": row.noeSanad == "PARDAKHT"? "پرداخت":row.noeSanad}</display:column>
    <display:column title="وضعیت" style="">${row.vaziat == "MOVAGHAT"? "موقت": row.vaziat == "DAEM"?"دائم":row.vaziat}</display:column>
    <display:column property="createdDate" title="زمان ثبت" style=""/>
    <display:column title="عملیات" style="">
        <input type="button" onclick="window.location='/fin/viewKhateSanadHa?sanad.id=${row.id}'" value="نمایش خط سند ها"/>
    </display:column>
</display:table>
